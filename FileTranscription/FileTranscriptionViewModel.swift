...
class FileTranscriptionViewModel {
	...
	private let logger = Logger(
		subsystem: Bundle.main.bundleIdentifier ?? "Whispera", category: "FileTranscriptionViewModel")
			...
			if let fileSize = attributes[.size] as? Int64 {
				...
				if fileSize > maxSizeBytes {
					throw FileTranscriptionError.transcriptionFailed(
						"File too large: \(fileSize) bytes (maximum: \(maxSizeBytes) bytes)")
				}
			}
		} catch {
			throw FileTranscriptionError.transcriptionFailed("File not found: \(url.path)")
	...
	private func performTranscription() async {
		...
		for (index, fileURL) in selectedFiles.enumerated() {
			...
			do {
				...
				logger.error(
					"Failed to transcribe \(fileURL.lastPathComponent): \(error.localizedDescription)")
...
struct FileTranscriptionResult: Identifiable {
	...
	init(
		fileURL: URL, filename: String, text: String, segments: [TranscriptionSegment]?,
