...
class YouTubeTranscriptionManager: YouTubeTranscriptionCapable {
	...
	// MARK: - Settings
	@ObservationIgnored @AppStorage("youtubeQuality") private var preferredQuality: YouTubeQuality =
		.medium
	...
	func transcribeSegment(at url: URL, startTime: Double, endTime: Double) async throws -> String {
		if isYouTubeURL(url) {
			...
			return try await fileTranscriptionManager.transcribeSegment(
				at: url, startTime: startTime, endTime: endTime)
		}
	}
	...
	func transcribeYouTubeURLWithTimestamps(_ url: URL) async throws -> [TranscriptionSegment] {
		...
		do {
			...
			let segments = try await fileTranscriptionManager.transcribeFileWithTimestamps(
				at: downloadedFile)
			progress = 1.0
	...
	private func downloadAudioStream(from streamURL: URL) async throws -> URL {
		...
		return try await networkDownloader.downloadFileWithChunks(
			from: streamURL, chunkSize: 2_097_152, preferredFileName: preferredName)  // 2MB chunks
	}
...
enum YouTubeTranscriptionError: LocalizedError {
	...
	var recoverySuggestion: String? {
		...
		case .videoIDExtractionFailed:
			return
				"Make sure the YouTube URL format is correct (e.g., youtube.com/watch?v=... or youtu.be/...)."
		case .videoInfoRetrievalFailed:
