...
final class WhisperKitTranscriberTests: XCTestCase {
	...
	func testSelectedModelSyncsWithCurrentModelAfterLoading() async throws {
		...
		XCTAssertEqual(
			storedModel, testModel, "selectedModel in UserDefaults should match the loaded model")
		XCTAssertEqual(transcriber.currentModel, testModel, "currentModel should be set correctly")
		XCTAssertEqual(
			transcriber.selectedModel, testModel, "selectedModel should match currentModel after loading")
	}
	...
	func testAudioArrayVsFileTranscriptionConsistency() async throws {
		...
		XCTAssertTrue(
			transcriber.responds(to: #selector(getter: WhisperKitTranscriber.isInitialized)),
			"Should have isInitialized property")
	}
	...
	func testDecodingOptionsForAudioArray() async throws {
		...
		// Then
		XCTAssertNotEqual(
			decodingOptionsTranscribe.task, decodingOptionsTranslate.task,
			"Translation mode should affect decoding options")
	...
	func testIsCurrentModelLoadedReflectsActualState() async throws {
		...
		// Then
		XCTAssertFalse(
			transcriber.isCurrentModelLoaded(), "Should return false when no model is loaded")
