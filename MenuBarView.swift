...
struct MenuBarView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 16) {
                ...
                VStack(spacing: 8) {
                    ...
                    #if DEBUG
                    Button("Test Crash (Debug Only)") {
                        // This will trigger SIGABRT and be caught by our signal handler
                        let array: [Int] = []
                        _ = array[10] // Index out of bounds - will crash with SIGABRT
                    }
        ...
        .overlay(
            ZStack {
                // Subtle drop zone indicator (always visible)
                if fileDropHandler != nil && !(fileDropHandler?.isDragging ?? false) {
                    VStack {
                        Spacer()
                        HStack(spacing: 4) {
