	...
	var body: some View {
		VStack(spacing: 24) {
			VStack(spacing: 16) {
				...
				Text(
					"Choose keyboard shortcuts to quickly start recording or transcribe files from anywhere."
				)
				.font(.body)
			...
			VStack(spacing: 20) {
				...
				VStack(spacing: 12) {
					...
					if showingShortcutCapture {
						ShortcutOptionsView(
							customShortcut: $customShortcut, showingOptions: $showingShortcutCapture)
					}
				}
